...
if __name__ == "__main__":
    ...
    results_path = os.path.join(args.base_path, "val_results.csv")
    ...
    save_path = os.path.join(args.base_path, "confusion_matrix.png")
    ...
    metric(results["label"], results["pred"], classes_for_cm, hyperparams)
    plt.savefig(save_path)
    
    # compute confusion matrix with predictions with probability higher than 0.75
    results = results.loc[results.pred_prob >= 0.75,]
    save_path_75 = os.path.join(args.base_path, "confusion_matrix_75.png")
    metric(results["label"], results["pred"], classes_for_cm, hyperparams)
