...
# load results
results_2 <- lapply(list.dirs(paste0(mount_path,"immucan_volume/processed_data/Panel_1/CellSighter/ParamSweep2/")),function(x){
...
results_1 <- lapply(list.dirs(paste0(mount_path,"immucan_volume/processed_data/Panel_1/CellSighter/ParamSweep/")),function(x){
  if(file.exists(paste0(x,"/val_results.csv"))){
    output <- str_extract(x,"Model_[0-9]{1,2}")
    if (output %in% unique(results$Output_name)){
      df <- read.csv(paste0(x,"/val_results.csv"))
      df$Output_name <- rep(output,nrow(df))
      df$Param_sweep <- rep(1,nrow(df))
      df <- df[,-c(1)]
      return (df)  
      }
    }
...
results <- bind_rows(results,results_1)
results$label <- as.character(results$label)

# load hyperparam dataframe
hyperparams_1 <- read.csv("data/hyperparams.csv")
hyperparams_1$X <- NULL
hyperparams_1$Param_sweep <- rep(1,nrow(hyperparams_1))
hyperparams_1$crop_input_size <- as.factor(hyperparams_1$crop_input_sizue)
# correct spelling mistake
hyperparams_1$crop_input_sizue <- NULL

hyperparams_2 <- read.csv("data/hyperparams2.csv")
hyperparams_2$X <- NULL
hyperparams_2$Param_sweep <- rep(2,nrow(hyperparams_2))

hyperparams <- rbind(hyperparams_1,hyperparams_2)


rownames(hyperparams) <- paste0(hyperparams$Model_name,hyperparams$Param_sweep)
...
col_vec_epoch_max = brewer.pal(name = "Set3", n = length(unique(hyperparams$epoch_max)))[1]
...
ha = rowAnnotation(df = hyperparams[hyperparams$Model_name %in% rownames(f1_matrix),]%>%
                     filter(Param_sweep == 2)%>%
                     select(-Model_name),
...
ggplot(results%>%
         group_by(label,crop_input_size)%>%
         summarise(radius = mean(sqrt(area/pi)),accuracy = sum(correct)/n(), crop_input_size = unique(crop_input_size))%>%
         filter(!label%in%c("Unlabelled","Tumor")),
       aes(x = radius, y= accuracy, col = as.factor(crop_input_size)))+
  geom_point() +
  theme_bw() +
  geom_smooth(method="lm")
