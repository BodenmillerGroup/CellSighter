---
title: "Prepare Parameter Sweep with 5-fold Validation"
output:
...
table(sce$k_fold_group,sce$cell_labels)
```
...
```{r}
if(!dir.exists(paste0(mount_path,"immucan_volume/processed_data/Panel_1/CellSighter/ParamSweep"))){
    dir.create(paste0(mount_path,"immucan_volume/processed_data/Panel_1/CellSighter/ParamSweep"))
  }
  ...
  for (epoch in epoch_max){
    for (l in lr){
      for (hm_name in names(hierarchy_match)){
        ...
        if(!dir.exists(paste0(mount_path,"immucan_volume/processed_data/Panel_1/CellSighter/ParamSweep/Model_",count,"/"))){
          dir.create(paste0(mount_path,"immucan_volume/processed_data/Panel_1/CellSighter/ParamSweep/Model_",count,"/"))
        }
        
        data2config(paste0(mount_path,"immucan_volume/processed_data/Panel_1/CellSighter/ParamSweep/Model_",count,"/"),
...
write.csv(df, file="data/hyperparams.csv")
