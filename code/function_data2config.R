...
data2config <- function(path_to_store, root_dir, crop_input_size=NULL ,train_split=0.8, train_set=NULL,
                        val_set=NULL, epoch_max=50,lr=0.001, hierarchy_match=NULL,blacklist=NULL,channels_path=paste0(root_dir,"/channels.txt"),
                        weight_to_eval=NULL, sample_batch=TRUE, to_pad=FALSE, size_data=1000, aug = TRUE,seed=060323){
  ...
  if (is.null(hierarchy_match)){
    ...
    hierarchy_match_string <- ""
    for (label in names(hierarchy_match[hierarchy_match!="-1"])){
      hierarchy_match_string <- paste0(hierarchy_match_string,"\"",label,"\":","\"",hierarchy_match[label],"\", ")
    }
    hierarchy_match_string <- substring(hierarchy_match_string,1,nchar(hierarchy_match_string)-2)
    hierarchy_match_string <- paste0("{",hierarchy_match_string,"}")
  }
  ...
  if (is.null(weight_to_eval)){
    weight_to_eval_string <- paste0("\"",path_to_store,"weights_",epoch_max-1,"_count.pth","\"")
  }
